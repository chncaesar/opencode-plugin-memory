...
After adding, the memory is immediately visible in future session system prompts.`,
        args: {
            title: z.string().max(120).describe("Short one-line title shown in the summary. Must fully represent the content: a future LLM reading only the title must be able to decide whether to load this entry. " +
...
If you are unsure of the exact ID, call memory_read first to find it.`,
        args: {
            ...
            title: z.string().max(120).optional().describe("New title (leave empty to keep existing). Must fully represent the content — see memory_add for granularity rules."),
